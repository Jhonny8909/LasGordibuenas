 ...
 */
async function ratesAtCheckoutUpdateDefaultParcelTemplate(client, objectId, options) {
    const input = {
        objectId: objectId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => components.DefaultParcelTemplateUpdateRequest$outboundSchema.optional()
    ...
    const [result] = await M.match(M.json(200, components.DefaultParcelTemplate$inboundSchema), M.fail([400, "4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
