 ...
 */
async function trackingStatusCreate(client, carrier, trackingNumber, metadata, options) {
    const input = {
        carrier: carrier,
        metadata: metadata,
        trackingNumber: trackingNumber,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => components.TracksRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(200, components.Track$inboundSchema), M.fail([400, "4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
