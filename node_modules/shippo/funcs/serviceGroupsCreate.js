...
async function serviceGroupsCreate(client, request, options) {
    const input = request;
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => components.ServiceGroupCreateRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(201, components.ServiceGroup$inboundSchema), M.fail([400, "4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
