 ...
 */
async function batchesGet(client, batchId, options) {
    const input = {
        batchId: batchId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.GetBatchRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const path = (0, url_js_1.pathToFunc)("/batches/{BatchId}")(pathParams);
    ...
    const [result] = await M.match(M.json(200, components.Batch$inboundSchema), M.fail([400, "4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
}
exports.batchesGet = batchesGet;
//# sourceMappingURL=batchesGet.js.map