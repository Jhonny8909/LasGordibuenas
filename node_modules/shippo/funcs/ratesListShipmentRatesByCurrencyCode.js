...
exports.ratesListShipmentRatesByCurrencyCode = void 0;
 ...
 */
async function ratesListShipmentRatesByCurrencyCode(client, request, options) {
    const input = request;
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.ListShipmentRatesByCurrencyCodeRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const body = null;
    const pathParams = {
        CurrencyCode: (0, encodings_js_1.encodeSimple)("CurrencyCode", payload.CurrencyCode, {
            explode: false,
    ...
    const path = (0, url_js_1.pathToFunc)("/shipments/{ShipmentId}/rates/{CurrencyCode}")(pathParams);
    ...
    const [result] = await M.match(M.json(200, components.RatePaginatedList$inboundSchema), M.fail([400, "4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
}
exports.ratesListShipmentRatesByCurrencyCode = ratesListShipmentRatesByCurrencyCode;
//# sourceMappingURL=ratesListShipmentRatesByCurrencyCode.js.map